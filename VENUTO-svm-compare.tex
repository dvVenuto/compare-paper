...
\vskip -0.5cm
	...
	\caption{Geometric interpretation of paired comparison data and the decision boundaries of our support vector machine classifier. \textbf{Top}: input feature pairs
		...
		x_i)]\in\{-1,1\}$ in the difference space are the decision boundaries.}
\label{fig:norm-data}
\end{figure}
...
Our predictions for the baseline (trivial) model are calculated by predicting either a win or loss for every match:
\begin{equation}c_{\tau}(x,x')=
\begin{cases} 1 &
\text{ if } |
...
Since there are 3 labels $y\in\{-1,0,1\}$ corresponding to $\{$win, draw, loss$\}$ respectively, the trivial AUC is non-standard (not 0.5). In our trivial calculation, we obtain a FPR and TPR of 0 by predicting a negative class for every match (draw or $y=0$).  In this method, the features are not used at all for the prediction.  By predicting every match as the most common positive class (win or draw), we have obtained the maximum TPR without any learning.  We then obtain the FPR given all observations are predicted to be the most common positive prediction (win or draw).  Since we have 2 distinct positive classes, it is highly unlikely that our TPR and FPR will be 1.0 through this method.
