...
Our predictions for the baseline (trivial) model are therefore calculated with:
...
relevant in learning models that performed better than ELO scores.  Models using a combination of ELO and Glicko features obtain an AUC that appears to be the median of the Glicko and ELO AUC values.  The model appears to be learning a combination of these features and therefore obtains an AUC AUC that is between the AUC values of ELO or Glicko scores only.  The Gaussian kernel also has a lower AUC with respect to the linear and polynomial kernels.  The fact that the linear kernel is more accurate than the Gaussian kernel suggests that the pattern is linear in the features we have computed.
...
For future work, For future work, it will be interesting to explore other algorithms for learning ranking and comparison functions. For example, it will be interesting to explore online algorithms that can be updated in O(1) time for each new data point (e.g. the most recent chess match).  As SVM with SMO optimization is limited to relatively small data sets it would be interesting to to try algorithms based on
